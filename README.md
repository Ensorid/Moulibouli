# Moulibouli FORK FROM MINI-MOULINETTE

![mini-moulinette](mini-moulinette.jpg)

Moulibouli is a test runner for 42 assignments forked from [k11q's mini-moulinette](https://github.com/k11q/mini-moulinette), providing automated tests for whole assignment with one command. The goal is to make thorough checking before submitting the assignments, with ease.
...
- Moulibouli runs through all the tests cases automatically, and checks if the test conditions are met.
...
Moulibouli is updated from time to time. To update it:
...
> ***Warning***
> Moulibouli is not 100% accurate, the tests may not cover every edge cases like moulinette. Use with caution.
...
```bash
git clone https://github.com/Ensorid/Moulibouli.git ~/moulibouli
```
...
```zsh
echo "alias mouli='~/moulibouli/moulibouli.sh'" >> ~/.zshrc && source ~/.zshrc
```

- bash:

```bash
echo "alias mouli='~/moulibouli/moulibouli.sh'" >> ~/.bashrc && source ~/.bashrc
```
...
5. Run using command `mouli` with assignment number as argument. e.g: C02.
...
We welcome contributions to moulibouli! Here are a few ways you can help out:
...
- Features: Do you have an idea for a new feature that you think would be useful for moulibouli? We'd love to hear it! Please create an issue on our GitHub repository to start a discussion.
- Feedback: If you've used moulibouli and have some feedback or suggestions for improvement, please let us know! Your input is valuable to us, and we're always looking for ways to make our test runner better.

When contributing to moulibouli, please be sure to follow our code of conduct and our guidelines for submitting contributions. Thank you for your interest in our project, and we look forward to working with you!
